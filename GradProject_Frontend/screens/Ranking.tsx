...
const Movie = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            axios.get('http://10.0.2.2:3000/get_sf_ranking')
                .then(response => {
            ...
            axios.get('http://10.0.2.2:3000/get_major_ranking')
                .then(response => {
    ...
    return (
        <SafeAreaView>
            ...
            <View style={styles.listcontainer}>
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-start', paddingTop: 8, paddingBottom: 18, marginLeft: 10, marginTop: 10 }}>
                    <ToggleButton />
                </View>
            ...
            {view === "gridview" ? (
                    ...
                    renderItem={({ item, index }) => (
                        <View style={styles.gridItem}>
                            ...
                            <View style={styles.gridtextposition}>
                                ...
                                {index < 8 ? (<View style={styles.addButtonContainer}>
                                    <Button onPress={additem(item, index)} title="Add" />
                                </View>) : (<View style={styles.addLastButtonContainer}>
                                    <Button onPress={additem(item, index)} title="Add" />
                                </View>)}
                            </View>
                ...
                <SectionList
                    ...
                    renderItem={({ item, index }) => (
                        <View style={index != 9 ? styles.content : styles.lastcontent}>
                            <TouchableOpacity onPress={() => handleImagePress(item)}>
                                <Image style={styles.imagesize} source={{ uri: item.url }} />
                            </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    addButtonContainer: {
        borderRadius: 20,
        marginBottom: 20, // add a margin around the button
    },
    addLastButtonContainer: {
        borderRadius: 20,
        marginBottom: 70, // add a margin around the button
    },
    modalContainer: {
        flex: 1,
        justifyContent: 'center',
